...
# Configuración
num_simulaciones = 10000 # Número de puntos a generar

@profile # Para el analísis
def estimar_pi(num_simulaciones):
    ...
    # Estimación de pi (proporcionalidad lineal)
    pi_estimado = 4 * (puntos_dentro / num_simulaciones)
    print(f"Valor estimado de pi: {pi_estimado}")

    # Visualización de los puntos en una gráfica (si están dentro son azules, si están fuera son rojos)
...
=============================================================
     9     70.5 MiB     70.5 MiB           1   @profile
    10                                         def estimar_pi(num_simulaciones):
    11     70.5 MiB      0.0 MiB           1       puntos_dentro = 0
    12     70.5 MiB      0.0 MiB           1       puntos_x_dentro = []
    13     70.5 MiB      0.0 MiB           1       puntos_y_dentro = []
    14     70.5 MiB      0.0 MiB           1       puntos_x_fuera = []
    15     70.5 MiB      0.0 MiB           1       puntos_y_fuera = []
    16
    17                                             # Generar puntos y contar cuántos caen dentro del círculo
    18     77.8 MiB -31506.5 MiB      100001       for _ in range(num_simulaciones):
    19     77.8 MiB -31532.4 MiB      100000           x = random.uniform(0, 1)
    20     77.8 MiB -31527.4 MiB      100000           y = random.uniform(0, 1)
    21     77.8 MiB -31530.9 MiB      100000           if x**2 + y**2 <= 1:
    22     77.8 MiB -24699.5 MiB       78418               puntos_dentro += 1
    23     77.8 MiB -24692.9 MiB       78418               puntos_x_dentro.append(x)
    24     77.8 MiB -24694.3 MiB       78418               puntos_y_dentro.append(y)
    25                                                 else:
    26     77.8 MiB  -6832.0 MiB       21582               puntos_x_fuera.append(x)
    27     77.8 MiB  -6807.4 MiB       21582               puntos_y_fuera.append(y)
    28
    29                                             # Estimación de pi
    30     77.0 MiB     -0.8 MiB           1       pi_estimado = 4 * (puntos_dentro / num_simulaciones)
    31     77.0 MiB      0.0 MiB           1       print(f"Valor estimado de pi: {pi_estimado}")
    32
    33                                             # Visualización
    34     93.7 MiB     16.7 MiB           1       plt.figure(figsize=(8, 8))
    35    100.6 MiB      6.9 MiB           1       plt.scatter(puntos_x_dentro, puntos_y_dentro, color='blue', s=1, label='Dentro del círculo')
    36     97.3 MiB     -3.3 MiB           1       plt.scatter(puntos_x_fuera, puntos_y_fuera, color='red', s=1, label='Fuera del círculo')
    37
    38                                             # Dibujar el círculo
    39     97.3 MiB      0.0 MiB           1       circle = plt.Circle((0, 0), 1, color='black', fill=False, linestyle='--')   
    40     97.3 MiB      0.0 MiB           1       plt.gca().add_artist(circle)
    41
    42                                             # Configuración del gráfico
    43     97.3 MiB      0.0 MiB           1       plt.title(f"Estimación de Pi usando Monte Carlo (Pi ≈ {pi_estimado:.4f})")  
    44     97.3 MiB      0.0 MiB           1       plt.xlabel("x")
    45     97.3 MiB      0.0 MiB           1       plt.ylabel("y")
    46     97.4 MiB      0.1 MiB           1       plt.legend()
    47     97.4 MiB      0.0 MiB           1       plt.axis('equal')
    48     97.4 MiB      0.0 MiB           1       plt.xlim(0, 1)
    49     97.4 MiB      0.0 MiB           1       plt.ylim(0, 1)
    50     97.4 MiB      0.0 MiB           1       plt.grid(True)
    51
    52    103.9 MiB      6.5 MiB           1       plt.show()
    53    103.9 MiB      0.0 MiB           1       return pi_estimado


Tiempo de ejecución: 46.6897 segundos"""
